...
<?php	if(!empty($_GET['id'])){ ?>
		<?php
			extract($_GET); 
			$k = $koneksi->query("SELECT * FROM produk where id='$id'"); 
			$data = $k->fetch_array();
		?>
		<div class="col-md-9">
			<div class="row">
			...
			<h3>Detail : <?php echo $data['nama'] ?></h3>
				...
				<?php $kat = mysqli_fetch_array($koneksi->query("SELECT * FROM kategori_produk where id='$data[kategori_produk_id]'"));  ?>
					...
					<a href="<?php echo $url; ?>menu.php?id=<?php echo $data['id'] ?>">
						...
						<img class="img-responsive" style="width: 100%; height: 300px;" src="<?php echo $url; ?>uploads/<?php echo $data['gambar'] ?>"> 
					</a>
					<br><br>
...
<?php }elseif(!empty($_GET['kategori'])){ ?>	
		...
		<?php
			extract($_GET); 
			$kat = mysqli_fetch_array($koneksi->query("SELECT * FROM kategori_produk where id='$kategori'")); 
		?>
		<div class="col-md-9">
			<div class="row">
			<div class="col-md-12">
			...
			<h3>Kategori : <?php echo $kat['nama'] ?></h3>
				<?php 
					$k = $koneksi->query("SELECT * FROM produk where kategori_produk_id='$kategori'");
					while($data = $k->fetch_array()){
				?>
				<div class="col-md-4 content-menu">
					<a href="<?php echo $url; ?>menu.php?id=<?php echo $data['id'] ?>">
						<img class="img-responsive" style="width: 100%; height: 200px;" src="<?php echo $url; ?>uploads/<?php echo $data['gambar'] ?>">
		...
		<div class="col-md-12"><hr>
			<h3>Daftar Semua Menu</h3>
				<?php 
					...
					$k = $koneksi->query("SELECT * FROM produk");
					while($data = $k->fetch_array()){
