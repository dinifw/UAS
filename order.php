...
?>
		...
		<?php
			if(!empty($_POST['form-order'])){
				...
				$kt = mysqli_fetch_object($koneksi->query("SELECT *FROM kota where id='$kota'"));
				if($difference->days >= 3){
					
					$q = $koneksi->query("INSERT INTO pesanan VALUES(NULL,'$tanggal_pesan','$tanggal_digunakan','$_SESSION[iam_user]','$nama','$alamat','$kt->nama','$kt->ongkir','$telephone','0','belum lunas')");
					if($q){

						$last = mysqli_fetch_array($koneksi->query("SELECT *FROM pesanan order by id DESC limit 1"));
						...
						foreach($cart as $id => $qty){
							...
							if(!empty($product)){
								...
								$ins = $koneksi->query("INSERT INTO detail_pesanan VALUES(NULL,'$product[id]','$qty','$last[id]')");
					...
					?>
					<div class="alert alert-danger">Pemesanan minimal<strong> 3 Hari sebelum hari H</strong>, terimakasih atas pengertian Anda</div>
					<?php
				}
			}
		?>
				...
				<hr>
				 <form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kota" required class="form-control">
							<?php
								$kota = $koneksi->query("SELECT * FROM kota");
								while($kp = $kota->fetch_array()){
							?>
			...
			<div class="col-md-12">
				...
				<thead>
				<tr style="background:darkgoldenrod;font-weight:bold;">
